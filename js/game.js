document.addEventListener('DOMContentLoaded', () => {
    ...
    let player = {
        ...
        dy: 0, gravity: GRAVITY, jumpPower: JUMP_POWER, grounded: true
    };
    ...
    // 👰 Невеста
    let brideActive = false, brideStepTimer = 0;
    let bride = {
        x: canvas.width,
    ...
    // 💍 Кольцо
    let ringActive = false, groomTransformed = false;
    ...
    function drawIdleScreen() {
        ...
        ctx.font = '26px serif';
        for (let i = 0; i < 6; i++) {
            ctx.fillText('💜', 70 + i * 95, 175 + Math.sin(t * 0.7 + i * 1.3) * 18);
        }
    ...
    function drawHeart(x, y, size) {
        if (heartImg?.complete) {
            ...
            ctx.bezierCurveTo(x, y + size * 0.8, x + size / 2, y + size * 0.6, x + size / 2, y + t);
    ...
    function collision(a, b) {
        const pad = 8;
        return a.x + pad < b.x + (b.width || b.size) - pad &&
               a.x + a.width - pad > b.x + pad &&
               a.y + pad < b.y + (b.height || b.size) - pad &&
    ...
    function spawnObstacle(now) {
        ...
        obstacles.push({
            x: canvas.width, y: GROUND_Y - 35, width: 35, height: 35,
    ...
    function spawnHeart() {
        if (Math.random() < 0.018) {
            hearts.push({ x: canvas.width, y: GROUND_Y - 85 - Math.random() * 45, size: 38 });
        }
    }
    ...
    function resetGame() {
        obstacles = []; hearts = []; score = 0; gameSpeed = 3;
        brideActive = false; ringActive = false; groomTransformed = false;
    ...
    // 🔥 Полноэкранный режим (только мобильные)
    function isMobile() {
    ...
    function enterFullscreen() {
        ...
        if (isIOS) {
            ...
            document.querySelectorAll('.section').forEach(section => {
                if (!section.contains(container)) section.style.visibility = 'hidden';
            });
        } else {
    ...
    canvas.addEventListener('mousedown', (e) => {
        if (gameRunning) { e.preventDefault(); jump(); }
    });
